...
def test_quiet_checked_before_cap_and_no_record() -> None:
    ...
    cfg = pacing.resolve(Config(max_actions_per_hour=12))
...
def test_fast_mode_zeroes_jitter_lifts_cap_disables_quiet() -> None:
    cfg = pacing.resolve(Config(pacing_mode="fast", max_actions_per_hour=1, reply_delay_sec=[1, 3]))
    assert cfg.cap == 60
...
def test_publish_reserves_and_caps(make_ctx, store) -> None:
    cfg_obj = Config(max_actions_per_hour=1, quiet_hours=(0, 0))
