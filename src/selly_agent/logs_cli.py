...
def _open_web(args: argparse.Namespace) -> int:
    """`selly-agent logs --web` — print the tail's URL, then open it."""
    ...
    try:
        ...
        print(
            "selly-agent: the daemon serves the web tail — start it with "
            "`selly-agent daemon start` (or use `selly-agent logs` instead)",
            file=sys.stderr,
        )
        return 1
    if status != 200:
