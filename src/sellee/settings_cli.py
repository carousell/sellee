"""`sellee settings list|set|approve|cancel|undo` — the attended settings door.

Harness-independent: these verbs talk to the running daemon over its control routes (attended
bearer from the config-dir secret), never writing sellee.db directly. The door is the same trust as
...
def set_setting(port: int, token: str, key: str, value: str) -> int:
    ...
    try:
        ...
        return 1
    if status != 200:
        print(f"sellee: {result.get('error', 'could not set that')}", file=sys.stderr)
        return 1
...
def _decide(port: int, token: str, action: str, change_id: str) -> int:
    try:
        ...
        return 1
    if status != 200:
        print(f"sellee: {result.get('error', 'that was refused')}", file=sys.stderr)
        return 1
    print(result.get("message", result.get("status", "done")))
