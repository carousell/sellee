...
`pass run` posts to the daemon's control route rather than writing sellee.db directly — one writer
...
from sellee import config, control, paths, skills
...
def run(args) -> int:
    """`sellee pass run <type> --item <id> [--market <id>] [--follow]`."""
    ...
    try:
        ...
        return 1
    if status != 200:
        print(f"sellee: {resp.get('error', 'could not enqueue that pass')}", file=sys.stderr)
        return 1
...
def _follow(port: int, token: str, pass_id: str) -> None:
    ...
    while True:
        ...
        if status != 200:
            ...
            print(f"sellee: {data.get('error', f'HTTP {status}')}", file=sys.stderr)
            return
...
def _skills_dir() -> Path:
    ...
    """
    via_current = paths.current() / "src" / "sellee" / "skills"
...
This session is connected to the running sellee daemon over MCP. Its tools are the only way
...
`/sell` list something · `/catchup` what needs you · `/sellee` status and settings ·
...
_SKILL_BLURBS = (
    ("sellee-conventions", "the house rules — tools only, secrets stay dark, escalate when unsure"),
...
def harness_config(directory=None, *, quiet: bool = False) -> int:
    """`sellee harness config --attended [--dir DIR]` — write the attended session's config:
...
def chat(args=None) -> int:
    """`sellee chat` — regenerate the attended workspace, then become a Claude Code session.
    ...
    try:
        ...
        print(
            "sellee: the daemon is not running — start it with `sellee daemon start`",
            file=sys.stderr,
        )
        return 1
    if status != 200:
    ...
    if binary is None:
        print(
            "sellee: claude binary not found — set claude_bin in config or add it to PATH",
            file=sys.stderr,
        )
        return 1
...
def provision(args) -> int:
    """`sellee provision carousell-ai [--region XX]` — obtain the guest API key."""
