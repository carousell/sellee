"""`sellee update` — fetch, verify, swap, restart, check, and roll back if it went wrong.
...
The schema is the reason rollback is more than a symlink flip. A new version migrates sellee.db at
...
from sellee import __version__, config, deployment, heartbeat, paths, supervisor
from sellee.installer import materialize

log = logging.getLogger(__name__)

DEFAULT_BASE_URL = "https://github.com/carousell/sellee/releases/latest/download"

_SUMS_NAME = "SHA256SUMS"
_ASSET_RE = re.compile(r"^sellee-(?P<version>.+)\.tar\.gz$")
...
def release_from_sums(text: str) -> Release:
    ...
    if not candidates:
        raise UpdateError(f"no sellee archive listed in {_SUMS_NAME}")
    if len(candidates) > 1:
...
def latest_snapshot():
    """The newest pre-migration snapshot of sellee.db, or None when there is none.
    ...
    if not backups.is_dir():
        return None
    found = sorted(backups.glob("sellee-*-pre-*.db"), key=lambda p: p.stat().st_mtime)
    return found[-1] if found else None


def restore_snapshot(snapshot: Path) -> None:
    """Put a snapshot back as sellee.db. The daemon must be stopped: this replaces the file it
    would otherwise be holding open."""
    shutil.copy2(snapshot, paths.sellee_db())
    ...
    for suffix in ("-wal", "-shm"):
        sidecar = paths.sellee_db().with_name(paths.sellee_db().name + suffix)
        if sidecar.exists():
            sidecar.unlink()
...
UPDATE_NOTICE = "sellee {version} is out (you're on {current}). Update when it suits you: {how}."
...
def perform(args, cfg, out, *, platform=None) -> int:
    ...
    release = discover(base)
    out(f"sellee {__version__} → {release.version}")
    if not is_newer(release.version, __version__):
        out("Already up to date.")
        return 0
    ...
    if not was_running:
        ...
        supervisor.install(mode=mode, platform=platform)
        out(f"Updated to {release.version}. Start it to finish: sellee daemon start")
    ...
    if healthy:
        ...
        out(
            ...
            f"(sellee update --rollback to revert)."
        )
        return 0
...
def _clear_cache(*, keep) -> None:
    ...
    for stale in paths.cache_dir().glob("sellee-*.tar.gz"):
        if stale != keep:
...
def run(args) -> int:
    ...
    try:
        ...
        print(f"sellee: {exc}", file=sys.stderr)
        return 1
    except materialize.LayoutError as exc:
        print(f"sellee: {exc.message}", file=sys.stderr)
        if exc.fix:
