...
from sellee import deployment, passes, paths, supervisor
from sellee.browser import chrome
from sellee.browser import client as browser_client
from sellee.installer import checks, runtime
...
AGENT_ENV_KEYS = ("CLAUDECODE", "CLAUDE_CODE", "AI_AGENT", "SELLEE_DAEMON_PASS")
...
def check_platform() -> checks.Check:
    ...
    if name is None:
        return checks.fail(
            ...
            "sellee runs on macOS and Linux today; Windows is a planned port.",
        )
...
def check_systemd_user() -> checks.Check:
    ...
    return checks.fail(
        ...
        "sellee keeps its background worker alive with a systemd user unit, which needs a "
...
def check_tree_location(tree) -> checks.Check:
    ...
    if is_tcc_blocked(tree, roots):
        ...
        return checks.fail(
            ...
            "Move the tree somewhere unprotected (e.g. ~/dev/sellee) and re-run ./setup — "
