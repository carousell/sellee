...
RC_MARKER_START = "# >>> sellee >>>"
RC_MARKER_END = "# <<< sellee <<<"
...
def _guard_current_is_ours() -> None:
    ...
    if current.exists() and not current.is_symlink():
        raise LayoutError(
            f"{current} is a real directory, not the symlink sellee manages",
            f"Move or remove it (it was not created by sellee), then re-run:\n"
...
def stage_version(tree, version: str) -> Path:
    ...
    for name in VERSION_DIRS:
        ...
        if not source.is_dir():
            raise LayoutError(
                f"{tree} does not look like a sellee tree ({name}/ is missing)",
...
def shim_target() -> Path:
    ...
    return paths.current() / "bin" / "sellee"


def _shim_is_ours(shim: Path) -> bool:
    """Whether the name at `~/.local/bin/sellee` is one we installed.
...
def install_shim() -> Path:
    """Link `~/.local/bin/sellee` at the launcher inside `current`."""
    ...
    if (shim.is_symlink() or shim.exists()) and not _shim_is_ours(shim):
        ...
        raise LayoutError(
            f"{shim} already exists and was not created by sellee",
...
def layout_preview(platform=None) -> list:
    ...
    rows = [
        ...
        (paths.shim_path(), "the `sellee` command"),
        (
