"""`sellee mcp-proxy` — the stdio shim for harnesses that speak MCP only over stdio.
...
from sellee import config, secrets
...
def main(args) -> int:
    ...
    if not token:
        print(
            "sellee: no MCP token found — start the daemon first (sellee daemon run)",
            file=sys.stderr,
        )
        return 1
