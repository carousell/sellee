...
Each DB (data = sellee.db, events = events.db) has its own directory of NNNN_*.sql files and
...
half-migrated DB. sellee.db is snapshotted before its migrations run — but only when something
...
def _snapshot(db_path: Path, pre_version: int, backups_dir: Path, backups_keep: int) -> Path:
    """Copy sellee.db to a pre-migration backup via the sqlite3 backup API (never a file copy —
    a live WAL DB must not be byte-copied). Then prune to the newest `backups_keep`."""
    backups_dir.mkdir(parents=True, exist_ok=True)
    ts = int(time.time())
    dest = backups_dir / f"sellee-{ts}-pre-{pre_version:04d}.db"
...
def prune_backups(backups_dir: Path, backups_keep: int) -> None:
    """Keep the newest `backups_keep` sellee.db snapshots; remove the rest. Shared by the
    snapshot step here and the retention task, so the retention policy has one source."""
    backups = sorted(
        backups_dir.glob("sellee-*.db"),
